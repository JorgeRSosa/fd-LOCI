...
for(k in 1:length(rho)){
  ...
  for(l in 1:length(m_m)){
    ...
    {
      set.seed(i)
      muestra[[i]] <-  fdata(func.sim(n=n2,mdata = length(t),mu = m1(t),sigma = S,rho = rho[k],mu2 = m_m[[l]]),
                             argvals = argvals)
    }
    d100[[l]] <- muestra
  ...
  d100r[[k]] <- d100
  save(d100r,file = 'Rdata/muestra100D_magnitud_.Rdata')
    
  p100m[[k]] <- prob100
}

save(p100m,file = 'Rdata/prob100D_magnitud.RData')